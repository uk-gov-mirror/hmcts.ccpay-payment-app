...
public class PaymentController {
    ...
    private void populateApportionedFees(List<PaymentDto> paymentDtos, PaymentFeeLink paymentFeeLink, boolean apportionFeature, Payment payment, String paymentReference) {
        ...
        List<PaymentFee> fees1 = new ArrayList<>();
        final boolean isPaymentAfterApportionment = false;
        if (apportionCheck && apportionFeature) {
            ...
            Iterable<FeePayApportion> apportionList = paymentService.findAll();
            Iterable<PaymentFee> feeList = paymentFeeRepository.findAll();
            apportionList.forEach(feePayApportion ->
            {
                if(feePayApportion.getPaymentId().equals(payment.getId()))
                {
                    feeList.forEach(paymentFee ->
                    {
                        PaymentFee fee = new PaymentFee();

                        if(paymentFee.getId().equals(feePayApportion.getFeeId()))
                        {
        ...
        if(!fees1.isEmpty()) {
            paymentDto = paymentDtoMapper.toReconciliationResponseDtoForLibereta(payment, paymentReference, fees1, ff4j, isPaymentAfterApportionment);
        }
        else
        {
            paymentDto = paymentDtoMapper.toReconciliationResponseDtoForLibereta(payment, paymentReference, fees, ff4j, isPaymentAfterApportionment);
        }
